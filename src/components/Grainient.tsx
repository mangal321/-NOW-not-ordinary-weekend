...
/**
 * Grainient — animated WebGL gradient background (React Bits).
 * Renders a live GPU gradient on web; falls back to a static CSS gradient
 * if WebGL is unavailable (or anything fails), and a static color on native.
 * It can never crash the tree: all GL work is guarded.
 */
...
export default function Grainient({
  ...
  // Probe WebGL2 support exactly once (never during re-renders).
  const [glSupported] = useState(() => (Platform.OS === "web" ? webgl2Supported() : false));
  const useFallback = !isWeb || failed || !glSupported;

  // Effect 1: build WebGL context once, pause when offscreen / tab hidden.
  // Fully guarded — any failure flips to the static fallback instead of crashing.
  useEffect(() => {
    ...
    try {
      ...
      var cleanupVisibility = () => document.removeEventListener("visibilitychange", onVisibility);
    } catch {
      fail();
      try {
  ...
  }, [useFallback]); // renderer created once
  ...
  useEffect(() => {
    ...
    useFallback, timeSpeed, colorBalance, warpStrength, warpFrequency, warpSpeed,
  ...
  if (!isWeb) {
    return <View style={{ flex: 1, backgroundColor: color3 }} />;
  }
    ...
    return (
      <div
        ...
        style={{
          ...
          height: "100%",
          overflow: "hidden",
          background: `linear-gradient(135deg, ${color1} 0%, ${color2} 52%, ${color3} 100%)`,
          ...style,
        }}
      />
    );
